...
import DataAccessObject.ConexionSQL;
...
public class UsuarioDAO implements IUsuario<UsuarioDTO> {
    ...
    @Override
    public List<UsuarioDTO> listar (String columna, String texto) {
        List<UsuarioDTO> usuarios = new ArrayList();
        try {
            ps = CON.conectar().prepareStatement("SELECT * FROM Usuario WHERE " + columna + " = '%" + texto + "%'");
            rs = ps.executeQuery();
            while (rs.next()) {
